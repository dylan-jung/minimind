...
**소프트웨어/하드웨어 수준으로 극한까지 최적화된 LLM 만들기.**

Week 1~12는 LLM의 구조와 학습을 이해하는 기초 과정이다.
이후 양자화, 커스텀 커널(CUDA/Metal), Flash Attention, speculative decoding,
메모리 최적화, 추론 엔진 등을 다루며 직접 최적화된 LLM을 구현한다.

각 주차를 학습할 때 항상 **"이걸 어떻게 더 빠르고 효율적으로 할 수 있을까"**를 함께 생각한다.
